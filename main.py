...
def valida_prezzo(prezzo_unitario):
    try: 
        ...
        return None         
    if prezzo_unitario == 0 or prezzo_unitario < 0:
        messagebox.showerror("Errore", "Il prezzo unitario deve essere maggiore di 0")
        return None
...
def valida_quantita_mag(quantita_magazzino):
    try: 
        ...
        if quantita_magazzino < 0:
            messagebox.showwarning("Attenzione", "La quantità deve essere un valore numerico maggiore o uguale a 0.")
            return None
    except ValueError:
        messagebox.showwarning("Attenzione", "Quantità deve essere un valore numerico maggiore di 0")
        return None
...
def valida_quantita_vend(quantita_venduta):
    try:
        ...
        return None
    if quantita_venduta == 0 or quantita_venduta < 0 :
        messagebox.showerror("Errore", "La quantità venduta deve essere maggiore di 0")
        return None
...
def gestione_ordini():
    ...
    def inserisci_nuovo_ordine():
        ...
        aggiorna_tabella()
        nuovo_ordine=Ordine(id_ordine= id_ordine, id_prodotto= id_prodotto, id_cliente= id_cliente, quantita_venduta= int(quantita_venduta), importo= float(importo), data_ordine= data_ordine)
